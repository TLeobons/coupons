...
export default class Cart extends Component {
    ...
    handleCoupon = event => {
        const sale = 1 - (+event.target.value / 100)
        const coupon = event.target[event.target.selectedIndex].text
        this.props.handleCoupon(sale, coupon)
    }
    
    render() {
        console.log(this.props)
        console.log('subtotal', this.props.cart.subTotal)
        return (
